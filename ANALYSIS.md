...
**The dashboard itself, and how its headline claim can be checked rather than taken on faith.** The interactive Tableau dashboard (linked from the README) leads with the 8.5% MAE improvement as its headline, but doesn't stop there — hovering it surfaces the two numbers behind the percentage (Model MAE 19.21 EUR/MWh, Baseline MAE 21.00 EUR/MWh), so a skeptical reader can verify the arithmetic themselves instead of trusting a single derived stat. Below that sits the error-comparison chart with both MAE values drawn as reference lines, and the actual/predicted/baseline chart from Part 2's evidence section, styled with a matched color scheme so the same series reads the same color in both views.
